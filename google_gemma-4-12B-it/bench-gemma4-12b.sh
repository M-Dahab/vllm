...
OUT_DIR="${OUT_DIR:-/home/mohammad/vllm/google_gemma-4-12B-it/bench-results}"
PP="${PP:-1024}"
TG="${TG:-256}"
RUNS="${RUNS:-2}"
...
mkdir -p "$OUT_DIR"
TS="$(date +%Y%m%d-%H%M%S)"
OUT_JSON="$OUT_DIR/gemma4-12b-${TS}.json"
OUT_MD="$OUT_DIR/gemma4-12b-${TS}.md"
SUMMARY="$OUT_DIR/gemma4-12b-${TS}-summary.txt"

echo "Benchmarking $MODEL at $BASE_URL"
echo "pp=$PP tg=$TG runs=$RUNS concurrency=[$CONCURRENCY]"

# llama-benchy requires /v1 in --base-url.
uvx llama-benchy \
  --base-url "$BASE_URL" \
  --model "$MODEL" \
  --pp "$PP" \
...
for r in rows:
    ...
    if r[1] is not None and (best is None or r[1] > best[1]): best=r
if best:
